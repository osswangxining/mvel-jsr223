...
public class ExecutionStack {
  ...
  public Boolean peekBoolean() {
    if (size == 0) return null;
    if (element.value == null) return Boolean.FALSE;
    if (element.value instanceof Boolean) return (Boolean) element.value;
    if ((element.value instanceof Double) && Double.isNaN(((Double)element.value).doubleValue()))  return Boolean.FALSE;
    if ((element.value instanceof Number) && ((Number)element.value).doubleValue() == 0)  return Boolean.FALSE;
    if (element.value instanceof Object)  return Boolean.TRUE;
