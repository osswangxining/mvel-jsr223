...
//    return doOperations(__resolveType(val1.getClass()),
//            val1, operation,
//            __resolveType(val2.getClass()), val2);
  }
    ...
    if (operation < 20) {
      if (((type1 > 49 || operation == EQUAL || operation == NEQUAL) && type1 == type2) ||
        ...
        return doOperationsSameType(type1, val1, operation, val2);
    	  ...
    	  Double _double1 = new Double(0);
    	  Double _double2 = new Double(0);
    	  
    	  if(operation == EQUAL || operation == NEQUAL){
    		  if((val1 == null && val2 != null) || (val1 != null && val2 == null)){
    			  if(operation == EQUAL) return Boolean.FALSE;
	    		  if(operation == NEQUAL) return Boolean.TRUE;
    		  }
    		  try{
    	    	   _double1 = getNumber(val1, type1);
    	    	   _double2 =  getNumber(val2, type2);
	    	  }catch(Exception e){
	    		  if((type1 == DataTypes.W_BOOLEAN && !(Boolean)val1 && val2.toString().trim().length() == 0)
	    				  ||(type2 == DataTypes.W_BOOLEAN && !(Boolean)val2 && val1.toString().trim().length() == 0)){
	    			    if(operation == EQUAL) return Boolean.TRUE;
	    			    if(operation == NEQUAL) return Boolean.FALSE;
	    		  }
	    		  if(operation == EQUAL) return Boolean.FALSE;
	    		  if(operation == NEQUAL) return Boolean.TRUE;
	    	  }
    		  if(operation == EQUAL) 
    			  return _double1.doubleValue() == _double2.doubleValue()? Boolean.TRUE : Boolean.FALSE;
    		  if(operation == NEQUAL)
    			  return _double1.doubleValue() == _double2.doubleValue()? Boolean.FALSE : Boolean.TRUE;
    	  }
    	  ...
    	  if((type1 == DataTypes.OBJECT && type2 == DataTypes.W_BOOLEAN)|| (type2 == DataTypes.OBJECT && type1 == DataTypes.W_BOOLEAN)) {
    		  returnTarget = DataTypes.W_DOUBLE;
    	  }
    	  
    	  
        return doPrimWrapperArithmetic(_double1,
            operation,
            _double2, true, returnTarget);
      }
      else if (operation != ADD &&
          (type1 == DataTypes.W_BOOLEAN || type2 == DataTypes.W_BOOLEAN) &&
          type1 != type2 && type1 != EMPTY && type2 != EMPTY) {
    	  
    	  if(type1 != DataTypes.W_BOOLEAN){
    		  Boolean _b1 = ((val1 != null) && val1.toString().equalsIgnoreCase("1"));
    		  return doOperationNonNumeric(type1, _b1, operation, convert(val2, Boolean.class));
    	  }
    	  
    	  if(type2 != DataTypes.W_BOOLEAN){
    		  Boolean _b2 = ((val1 != null) && val2.toString().equalsIgnoreCase("1"));
    		  return doOperationNonNumeric(type1, convert(val1, Boolean.class), operation, _b2);
    	  }
      ...
      //Fix for: 减法运算符（-）:某个运算符不是数字，那么结果为 NaN。
      else if((operation == Operator.SUB || operation == Operator.MULT || operation == Operator.DIV || operation == Operator.MOD) ){
    	  Double _double1 = new Double(0);
    	  Double _double2 = new Double(0);
    	  try{
    	   _double1 = getNumber(val1, type1);
    	  ...
    	  if(type1 == DataTypes.W_BOOLEAN){
    		 val1 = Boolean.parseBoolean(val1.toString()) ? new Double(1) : new Double(0);
    	  }
    	  if(type2 == DataTypes.W_BOOLEAN){
     		 val2 = Boolean.parseBoolean(val2.toString()) ? new Double(1) : new Double(0);
     	  }
    	  _doOperations(type1, val1, operation, type2, val2);
      }
    }
  ...
  private static boolean isNumericOperation(int type1, Object val1, int operation, int type2, Object val2) {
	  boolean b1 = (type1 > 99 && type2 > 99)
		        || (operation != ADD && (type1 > 99 || type2 > 99 || operation < LTHAN || operation > GETHAN) && isNumber(val1) && isNumber(val2));
	  boolean b2 = ((type1 > 99 && val2 ==null) || (type2 > 99 && val1 == null)) && (operation == ADD ||operation == EQUAL ||operation == NEQUAL ||operation == SUB 
			  ||operation == DIV ||operation == MULT ||operation == MOD ||operation == GTHAN ||operation == GETHAN
			  ||operation == LTHAN ||operation == LETHAN ||operation == SOUNDEX);
	  boolean b3 = ((type1 == 0 && val1 ==null) || (type2 == 0 && val2 == null)) && (operation == SUB 
			  ||operation == DIV ||operation == MULT ||operation == MOD 
			  ||operation == SOUNDEX);
	  boolean b4 = ((type1 == DataTypes.W_BOOLEAN  && val2 ==null) || (type2 == DataTypes.W_BOOLEAN && val1 == null)) && (operation == ADD || operation == SUB 
			  ||operation == DIV ||operation == MULT ||operation == MOD 
			  ||operation == SOUNDEX);
	  boolean b5 = (type1 > 99 && type2 == DataTypes.W_BOOLEAN) || (type1 == DataTypes.W_BOOLEAN && type2 > 99);
	  boolean b6 = (type1 > 99 || type1 == DataTypes.W_BOOLEAN || type2 > 99 || type2 == DataTypes.W_BOOLEAN) && (operation == EQUAL ||operation == NEQUAL );
  ...
  private static Object doOperationNonNumeric(int type1, final Object val1, final int operation, final Object val2) {
    switch (operation) {
      ...
      case MOD:
      case GTHAN:
    	//wangxn: if one of them is Number, then will return false
